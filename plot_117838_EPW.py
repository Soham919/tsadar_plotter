...
def plot_117838_EPW(data):
    lp = data['learned_parameters.csv']
    r = shot_data.loc[117838,"pointing"] - lp["Radius (\\mum)"]/10**3
    bv = (r*10**2)/shot_data.loc[117838,"timing"]
...
def plot_117838_EPW_loss(data):
    lp_1 = data['117838-12']['learned_parameters.csv']   # learned parameters 1
    lp_2 = data['117838-15']['learned_parameters.csv']   # learned parameters 2
    r1 = shot_data.loc[117838,"pointing"] - lp_1["Radius (\\mum)"]/10**3   # radius 1
    r2 = shot_data.loc[117838,"pointing"] - lp_2["Radius (\\mum)"]/10**3   # radius 1
    loss_1 = data['117838-12']['losses.csv']   # loss 1
    loss_2 = data['117838-15']['losses.csv']   # loss 2
    ...
    ax2[0].scatter(lp_1["Radius (\\mum)"],loss_1["initial_losses"],facecolors='none', edgecolors='blue',label='117838-12')
    ax2[0].scatter(lp_2["Radius (\\mum)"],loss_2["initial_losses"],facecolors='none', edgecolors='red',label='117838-15')
    ax2[0].set_title("loss v.s r")
    ax2[0].set_xlabel(r'$r (\mu m)$')
    ax2[0].set_ylabel('loss')
    ax2[0].legend()

    ax2[1].plot(lp_1["Radius (\\mum)"],cl1,color='blue',linestyle='--',label='117838-12')
    ax2[1].plot(lp_2["Radius (\\mum)"],cl2,color='red',linestyle='--',label='117838-15')
    ax2[1].set_title("cumulative loss v.s r")
    ax2[1].set_xlabel(r'$r (\mu m)$')
    ax2[1].set_ylabel('cum.loss')
    ax2[1].legend()

def plot_117838_IAW(data):
    ### PLOT LEARNED PARAMETERS for H2 AND HE SHOCKS ###
    im1 = data['117838']['117838-15']['learned_parameters.csv']
    ...
    r = shot_data.loc[117838,"pointing"] - im1["Radius (\\mum)"]/10**3
    #bv = (r*10**2)/shot_data.loc[92538,"timing"]
    ## Ti, Va, fract plot for all 3 species 
    plt.rcParams.update({
    #    "font.family": "serif",             # pick font family
    #    "font.serif": ["Arial"],  # pick specific font
