...
def interp_Prz(P_rz, r_grid, z_grid,
               ...
               fill_value=np.nan):
    """
    Build interpolators for P(z, r) and P(x, y, z).

    P_rz:
        2D array with shape (len(z_grid), len(r_grid)).
        If given as (len(r_grid), len(z_grid)), it is auto-transposed.

    r_grid:
        1D increasing array of r values.

    z_grid:
        1D increasing array of z values.
    ...
    if P_rz.shape != expected_shape:
        raise ValueError(
            f"P_rz must have shape {expected_shape}, got {P_rz.shape}"
        )
