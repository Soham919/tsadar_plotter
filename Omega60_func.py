...
def gj_dens(p : float,M : float,D : float,x : float):
    ...
    """
    y = 7/5     # adiabatic constant for monoatomic(5/3)/diatommic(7/3) gas
...
ax[0].plot(x,2*gj_dens(696,3.9,10,x),color='MediumSeaGreen',linewidth=2,label='2019')
ax[0].plot(x,2*gj_dens(1500,3.9,10,x),color='orange',linewidth=2,label='2026')

ax[0].set_xlim([0,max(x)])
ax[0].set_ylabel(r"$\rho (cm^{-3})$")
ax[0].set_xlabel("Distance from nozzle exit (mm)")

ax[1].plot(x,1000*2*mp*gj_dens(696,3.9,10,x),color='MediumSeaGreen',linewidth=2,label='2019')
ax[1].plot(x,1000*2*mp*gj_dens(1500,3.9,10,x),color='orange',linewidth=2,label='2026')
