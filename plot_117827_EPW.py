...
def plot_117827_EPW(data):
    lp = data['learned_parameters.csv']
    r = 4 + lp["Time (ps)"]/10**3
    bv = (r*10**2)/shot_data.loc[117827,"timing"]
    ...
    Te = lp["Te_electron"].rolling(window=window+3).mean()
    ne = lp["ne_electron"].rolling(window=window).mean()
    std_Te = lp["Te_electron"].rolling(window=window+3).std()
...
def plot_117827_EPW_loss(data):
    lp_1 = data['117827-5']['learned_parameters.csv']   # learned parameters 1
    lp_2 = data['117827-8']['learned_parameters.csv']   # learned parameters 2
    r1 = shot_data.loc[117827,"pointing"] - lp_1["Radius (\\mum)"]/10**3   # radius 1
    r2 = shot_data.loc[117827,"pointing"] - lp_2["Radius (\\mum)"]/10**3   # radius 1
    loss_1 = data['117827-5']['losses.csv']   # loss 1
    loss_2 = data['117827-8']['losses.csv']   # loss 2
